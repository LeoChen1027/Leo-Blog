...
public class TypeController {
    ...
    @PostMapping("/types")
    public String post(@Valid Type type, Errors errors, RedirectAttributes attributes) {
        Type type1=typeService.getTypeByName(type.getName());
        if (type1!=null){
            errors.rejectValue("name","nameError","不能添加重复分类");
        }
        if (errors.hasErrors()){
            return "admin/types-input";
        }
    ...
    public String editPost(@Valid Type type, Errors errors, @PathVariable Long id,RedirectAttributes attributes) {
        Type type1=typeService.getTypeByName(type.getName());
        if (type1!=null){
            errors.rejectValue("name","nameError","不能添加重复分类");
        }
    ...
    public String delete(@PathVariable Long id,RedirectAttributes attributes){
        ...
        attributes.addFlashAttribute("message", "删除 分类成功");
        return "redirect:/admin/types";
    }
}
